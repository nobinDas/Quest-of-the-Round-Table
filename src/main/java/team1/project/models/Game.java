...
public class Game {
    ...
    private static ArrayList<AdventureCard> adventureCardList = new ArrayList<>(); //116 for teams of 3 (-8 test cards, -3 bid Allies)
    private static  ArrayList<StoryCard> storyCardList = new ArrayList<>(); //28

    private ArrayList<AdventureCard> discardedAdventureCardList = new ArrayList<>();
    private ArrayList<StoryCard> discardedStoryCardList = new ArrayList<>();
    private static int currentPlayer = 0; // this is the current player of the game
    private static int currentStoryPlayer; // within a story
    private static int kingsRecognitionEventShields = 0;
    private static int winners = 0;
    private static Scanner scanner;
    public static int totalNumPlayers;
    public static StoryCard currentStoryCard;
    ...
    private ArrayList<ArrayList<AdventureCard>> questTotalStagesSponsor = new ArrayList<>();
    ...
    public void drawAdventureCard(String ID){
        ...
        AdventureCard newCard  = adventureCardList.get(0);
    ...
    public void selectAlly(String ID, String index){
        ...
        assert p != null;
        AdventureCard card = p.getInHandCards().get(Integer.parseInt(index));
    ...
    public void discardAdventureCards(String playerID, String[] cards){
        ...
        assert p != null;
        ArrayList<AdventureCard> tobeDisc = new ArrayList<>();
        int counter=0;
        System.out.println("playerID: " + playerID);
        System.out.println(p);
        System.out.println(p.getInHandCards());

        for(AdventureCard pls : p.getInHandCards()){
            tobeDisc.add(pls);
        }

        for (String i : cards){
            int index = Integer.parseInt(i);
            AdventureCard card = p.getInHandCards().get(index);
            discardedAdventureCardList.add(card);
     ...
     * @return
     */
    public StoryCard drawStoryCard(String ID){
    ...
    public void printCards(ArrayList<AdventureCard> cards){
    ...
    public void discardAdventureCard(Player p){
        ...
        System.out.println(p.getName()+", you have more than 12 in hand cards\nPlease discard the extra card\n");
        printCards(p.getInHandCards());
        System.out.print("Enter index from (0 - "+ (p.getInHandCards().size()-1)+" ):  ");
        int choice = scanner.nextInt();
        System.out.println("You have decided to discard card at index "+ choice+", name= \n"+ p.getInHandCards().get(choice).getName());
        AdventureCard card = p.getInHandCards().get(choice);
        discardedAdventureCardList.add(card);
        p.getInHandCards().remove(choice);
    }
...
//    }
     ...
     */
    public Player testStageFight(Player p, AdventureCard testCard) {
            ...
            for(var i = 0; i < playersInStory.size(); i++){
                ...
                if (questTotalStagesSponsor.get(m).get(0).getCardType() == ADVENTURE_TEST) {
                    ...
                    for (int v=0; v<test; v++){
                        ...
                        if(playersInStory.get(v).getName() !=s.getName()){
                            // remove the losers
...
//                }
        ...
        for (int i=0; i<playersInStory.size();i++){
            for (int j=0; j<playersInStory.get(i).getInPlayCards().size(); j++){
                AdventureCard ac= playersInStory.get(i).getInPlayCards().get(j);
                if(ac.getCardType() == ADVENTURE_AMOUR){
                    discardedAdventureCardList.add(ac);
    ...
    public int calculateStageBattlePoints(ArrayList<AdventureCard> stage){
    ...
    public void handOutAdventureCards(){
        ...
        for(var j=0; j < totalNumPlayers; j++){
            for(var i = 0; i < 12; i++){
                AdventureCard ac = adventureCardList.get(i);
...
//            }
    ...
    public void createStoryDeck(){
        ...
        //add Pox Event * 1
        storyCardList.add(buildStoryCard(STORY_EVENT, "Pox", -1, null, "All players except the player drawing this card lose 1 shield", -1, 4));
        count++;

        //add Plague Event * 1
        storyCardList.add(buildStoryCard(STORY_EVENT, "Plague", -1, null, "Drawer loses 2 shields if possible", -1, 5));
        count++;
...
//
    ...
    public void parseEventCardFrontend(String eventsName){
        ...
        if (eventName.equals("King's Recognition")){
            ...
            System.out.println("Current player is :" + players.get(a).getName());
            performPox(players.get(a));
...
//        }
    ...
    public void performKingsCallToArms() {
        ...
        for (Player p : players) {
            ...
            for (AdventureCard ac : p.getInHandCards()) {
