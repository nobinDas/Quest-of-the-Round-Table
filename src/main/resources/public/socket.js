    ...
    stompClient.connect({}, function (frame) {
        ...
        stompClient.subscribe("/topic/progress",function (response){
            ...
            if(data.cardType == "STORY_QUEST"){
                ...
                StoryCard = data;
                $("#storyCard").empty();
                $("#storyCard").append("<p>Card: "+data.name+", Type: "+data.cardType +"</br>Bonus sheild: "+data.bonusShields+"</p>");
        ...
        stompClient.subscribe("/topic/group/" + playerName+"/tournamentAdCardUpdate",function (response){
            ...
            for (let i = 0; i < data.length; i++){
                $("#deck").append("<tr><td>" + i + ". " + data[i].name + ", " + data[i].cardType + "</td></tr>");
            }
        ...
        stompClient.subscribe("/topic/group/"+ playerName +"/playQuest",function (response){
            ...
            // else if(data.playersInStory == null){
        ...
        stompClient.subscribe("/topic/group/event",function (response){
            ...
            for (let i = 0; i < players.inHandCards.length; i++){
                  $("#deck").append("<tr><td>" + i + ". " + players.inHandCards[i].name + ", " + players.inHandCards[i].cardType + "</td></tr>");
            }
            $("#ranks").empty();

                for (let i = 0; i < data.players.length; i++){
                    $("#ranks").append("<tr><td>" + i + ". " + data.players[i].name + ", " + data.players[i].rankCard.name + ", " +data.players[i].shields + "</td></tr>");
            }
        });
        ...
        success: function (data){
            ...
            for(let i=0;i<data.playersInStory.length;i++){
                ...
                if(data.playersInStory[i].id == parseInt($("#player_id").text())){
                    ...
                    console.log("i+1: "+i+" data.playersInStory.length: "+data.playersInStory.length);
...
function tournament(){
    ...
    $.ajax({
        ...
        success: function (data){
            ...
            console.log("*****player in story****: "+data.playersInStory);
            //ask other players
...
function responseNo(){
    ...
    if(Question == "who wants to play the Quest"){
        ...
        stompClient.send("/topic/group/"+nowAsking.toString()+"/TournamentPlay?", {}, JSON.stringify("Player doesn't want to play the tournament"));
    }
}
